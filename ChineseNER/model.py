...
class Model(object):
    ...
    def project_layer(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project"  if not name else name):
            with tf.variable_scope("hidden"):
                ...
                output = tf.reshape(lstm_outputs, shape=[-1, self.lstm_dim*2], name="output")
                hidden = tf.tanh(tf.nn.xw_plus_b(output, W, b),name="hidden")
            ...
            with tf.variable_scope("logits"):
                ...
                pred = tf.nn.xw_plus_b(hidden, W, b, name="pred")
            print('pred:')
            print(pred)
            return tf.reshape(pred, [-1, self.num_steps, self.num_tags], name="scores")
    ...
    def run_step(self, sess, is_train, batch):
        ...
        if is_train:
            ...
            # print(len([self.lengths, self.logits]))
            lengths, logits = sess.run([self.lengths, self.logits], feed_dict)
            # print(type(lengths))
            # print(lengths)
            # logitss = tf.Variable([lengths, logits], name='output')
            # return logitss
    ...
    def evaluate(self, sess, data_manager, id_to_tag):
        ...
        for batch in data_manager.iter_batch():
            ...
            for i in range(len(strings)):
                ...
                # print([id_to_tag[int(x)] for x in batch_paths[i][:lengths[i]]])
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        ...
        # logitss = self.run_step(sess, False, inputs)
        # lengths = logitss[0]
        # scores = logitss[1]
        # print(scores)
        batch_paths = self.decode(scores, lengths, trans)
        # print([id_to_tag[idx] for idx in batch_paths[0]])
